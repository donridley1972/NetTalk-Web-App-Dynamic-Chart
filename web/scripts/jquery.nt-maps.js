...
// array = [bidv, lat, lng, options, desc, descopen, formaction]
	addMarkersPolygon: function(name,ar) {
		var ll=[];
		for (var i=0; i < ar.length; i++){
			ll[i] = new L.LatLng(ar[i][1],ar[i][2])
		}
		return ll;
	},	
//------------------------------------------------------
	addMarkerPolygon: function(bidv,lat,lng,options,desc,descopen,action) {
		var _this = this;
...
//------------------------------------------------------
	addRouteToMap: function(options) {	
...
//------------------------------------------------------
	addMarkersToRoute: function(name,ar) {
		var ll=[];
		if(this.options.polygons[name]){
			for (var i=0; i < ar.length; i++){
				ll[i] = new L.LatLng(ar[i][1],ar[i][2])
			}
			L.polyline(ll).addTo(this.options.mapId);
		}
	},
//------------------------------------------------------
