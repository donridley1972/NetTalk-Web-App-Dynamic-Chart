...
(function($, window, undefined) {
    ...
    var options = {
		...
		// added for nettalk. Function to call when an item on the list is clicked on
		clickHook: function(parent, item, itemData, level) {  //nt
		   // there is no default implementation              //nt
		},													  //nt
		// added for nettalk. Function to call when an item on the list is double clicked on
		dblclickHook: function(parent, item, itemData, level) {  //nt
		   // there is no default implementation              //nt
		},													  //nt
    ...
    var aciTree_core = {
        ...
        init: function(options) {
            ...
            this._instance.jQuery.addClass('aciTree' + this._instance.index).attr('role', 'tree')
				.on('click' + this._instance.nameSpace, '.aciTreeButton', this.proxy(function(e) {
                // process click on button
				console.log('process click on button')
        ...
        _createItem: function(itemData, level) {
            if (this._private.itemClone[level]) {
				...
				//li.setAttribute('id', itemData.id); //nt
            ...
            if (itemData.icon) {
                if (itemData.icon instanceof Array) {
                    icon.className = 'aciTreeIconX ' + itemData.icon[0];
                    icon.style.backgroundPosition = itemData.icon[1] + 'px ' + itemData.icon[2] + 'px';
                } else {
                    icon.className = 'aciTreeIconX ' + itemData.icon;
                }
            } else {
        ...
        _createItems: function(ul, before, after, itemData, level, callback) {
            ...
            var task = new this._task(this._instance.queue, function(complete) {
				...
				//$('.aciTreeText').off('click.nt') //nt
				//	.on('click.nt',function(ev){
				//		_this._instance.options.clickHook(_this)						
				//	})
                    ...
                    for (var i = 0; i < itemData.length; i++) {
                        (function(itemData) {
                            task.push(function(complete) {
                                ...
                                items.push(item[0]);
								$(item[0]).find('.aciTreeText').off('click.nt').on('click.nt',function(ev){ //nt
									_this._instance.options.clickHook(parent, item, itemData, level)//nt
								})//nt
                                complete();
                            });
                        })(itemData[i]);
        ...
        addIcon: function(item, options) {
            ...
            if (this.isItem(item)) {
                ...
                var found = domApi.childrenByClass(parent, 'aciTreeIconX');
